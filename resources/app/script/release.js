...
function prepareAssets () {
  ...
  const zipAssets = [{
    name: 'trade-butler-gui-mac.zip',
    path: path.join(outPath, 'TradeButler -darwin-x64', 'TradeButler .app')
  }, {
    name: 'trade-butler-gui-windows.zip',
    path: path.join(outPath, 'TradeButler -win32-ia32')
  }, {
    name: 'trade-butler-gui-linux.zip',
    path: path.join(outPath, 'TradeButler -linux-x64')
  }]
  ...
  return Promise.all(zipAssets.map(zipAsset)).then((zipAssets) => {
    return zipAssets.concat([{
      ...
      name: 'TradeButlerGUI.exe',
      path: path.join(outPath, 'windows-installer', 'TradeButlerGUI.exe')
    }, {
      name: `trade-butler-gui-${version}-full.nupkg`,
      path: path.join(outPath, 'windows-installer', `trade-butler-gui-${version}-full.nupkg`)
    }])
  })
}
